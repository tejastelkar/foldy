# Foldy macOS App Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-09-10-foldy-design.md`
...
- `Foldy/App/FoldyApp.swift` — menu-bar app entry point and dependency construction.
- `Foldy/App/AppModel.swift` — main-actor UI state and commands.
- `Foldy/Domain/FoldState.swift` — normalized visual parameters.
- `Foldy/Domain/FoldStateMapper.swift` — angle-to-effect mapping and hysteresis.
- `Foldy/Sensor/LidAngleProviding.swift` — sensor stream abstraction.
- `Foldy/Sensor/AngleSmoother.swift` — noise reduction and stale-state detection.
- `Foldy/Sensor/HIDReportParser.swift` — pure three-byte feature-report decoder.
- `Foldy/Sensor/HIDAngleProvider.swift` — IOKit HID device discovery and polling.
- `Foldy/Sensor/PreviewAngleProvider.swift` — slider-driven unsupported-device source.
- `Foldy/Capture/ScreenCaptureServicing.swift` — capture abstraction and permission status.
- `Foldy/Capture/ScreenCaptureService.swift` — ScreenCaptureKit display stream.
- `Foldy/Rendering/FoldRenderParameters.swift` — GPU parameter layout.
- `Foldy/Rendering/FoldRenderer.swift` — Metal texture pipeline and failure reporting.
- `Foldy/Rendering/FoldShaders.metal` — perspective, crease, blur, shade, and dim shader.
- `Foldy/Overlay/OverlayCoordinating.swift` — overlay lifecycle abstraction.
- `Foldy/Overlay/OverlayCoordinator.swift` — click-through windows and fail-safe teardown.
- `Foldy/Overlay/FoldMetalView.swift` — MTKView bridge used by overlay windows.
- `Foldy/Settings/AppSettings.swift` — typed UserDefaults-backed preferences.
- `Foldy/Settings/LaunchAtLoginService.swift` — ServiceManagement wrapper.
- `Foldy/Entitlement/EntitlementStore.swift` — entitlement abstraction and state.
- `Foldy/Entitlement/LocalEntitlementStore.swift` — deterministic development unlock.
- `Foldy/Entitlement/DirectLicenseStore.swift` — signed license-file validation boundary.
- `Foldy/UI/MenuBarContentView.swift` — primary controls and status.
- `Foldy/UI/OnboardingView.swift` — first-run explanation, permission, compatibility.
- `Foldy/UI/PreviewView.swift` — angle slider and rendered demo.
- `Foldy/Resources/Assets.xcassets` — app accent and icon assets.
- `Foldy/Foldy.entitlements` — screen capture and hardened-runtime-compatible settings.
- `FoldyTests/*Tests.swift` — domain, parser, settings, entitlement, and safety tests.
...
**Files:**
- Create: `project.yml`
- Create: `Foldy/App/FoldyApp.swift`
- Create: `Foldy/App/AppModel.swift`
- Create: `Foldy/UI/MenuBarContentView.swift`
- Create: `Foldy/Resources/Assets.xcassets/Contents.json`
- Create: `Foldy/Resources/Assets.xcassets/AccentColor.colorset/Contents.json`
- Create: `Foldy/Foldy.entitlements`
- Test: `FoldyTests/AppModelTests.swift`

**Interfaces:**
- Consumes: none.
- Produces: `@MainActor final class AppModel: ObservableObject`, `func setEnabled(_ enabled: Bool)`, and the `Foldy` app target used by all later tasks.
...
Create `FoldyTests/AppModelTests.swift` with a main-actor test that constructs `AppModel()`, asserts `isEnabled == false`, calls `setEnabled(true)`, and asserts `isEnabled == true`.

- [ ] **Step 3: Create the project definition and shell**

Set organization identifier `com.foldy`, product bundle identifier `com.foldy.app`, deployment target `14.0`, Swift version `6.0`, app category `public.app-category.entertainment`, `LSUIElement = true`, and test host `Foldy`. Implement `FoldyApp` with `MenuBarExtra("Foldy", systemImage: "macbook")` and `MenuBarContentView` containing the enable toggle and a Quit button.
...
```bash
xcodegen generate
xcodebuild test -project Foldy.xcodeproj -scheme Foldy -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO
```
...
```bash
git add project.yml Foldy FoldyTests/AppModelTests.swift
git commit -m "feat: scaffold Foldy menu bar app"
```

### Task 2: Fold Mapping, Smoothing, and Safety State

**Files:**
- Create: `Foldy/Domain/FoldState.swift`
- Create: `Foldy/Domain/FoldStateMapper.swift`
- Create: `Foldy/Sensor/AngleSmoother.swift`
- Test: `FoldyTests/FoldStateMapperTests.swift`
- Test: `FoldyTests/AngleSmootherTests.swift`
...
Run: `xcodebuild test -project Foldy.xcodeproj -scheme Foldy -destination 'platform=macOS' -only-testing:FoldyTests/FoldStateMapperTests CODE_SIGNING_ALLOWED=NO`
...
Run: `xcodebuild test -project Foldy.xcodeproj -scheme Foldy -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO`
...
```bash
git add Foldy/Domain Foldy/Sensor/AngleSmoother.swift FoldyTests
...
**Files:**
- Create: `Foldy/Sensor/LidAngleProviding.swift`
- Create: `Foldy/Sensor/HIDReportParser.swift`
- Create: `Foldy/Sensor/HIDAngleProvider.swift`
- Create: `Foldy/Sensor/PreviewAngleProvider.swift`
- Test: `FoldyTests/HIDReportParserTests.swift`
- Test: `FoldyTests/PreviewAngleProviderTests.swift`
...
```bash
git add Foldy/Sensor FoldyTests
...
**Files:**
- Create: `Foldy/Rendering/FoldRenderParameters.swift`
- Create: `Foldy/Rendering/FoldRenderer.swift`
- Create: `Foldy/Rendering/FoldShaders.metal`
- Create: `Foldy/Overlay/FoldMetalView.swift`
- Create: `Foldy/UI/PreviewView.swift`
...
```bash
git add Foldy/Rendering Foldy/Overlay/FoldMetalView.swift Foldy/UI/PreviewView.swift FoldyTests
...
**Files:**
- Create: `Foldy/Capture/ScreenCaptureServicing.swift`
- Create: `Foldy/Capture/ScreenCaptureService.swift`
- Create: `Foldy/Overlay/OverlayCoordinating.swift`
- Create: `Foldy/Overlay/OverlayCoordinator.swift`
- Modify: `Foldy/Rendering/FoldRenderer.swift`
- Modify: `Foldy/App/AppModel.swift`
- Test: `FoldyTests/OverlaySafetyTests.swift`
...
Request shareable content, select the matching display, exclude Foldy-owned windows, configure `SCStreamConfiguration` for native pixel size and 30 FPS, and yield only complete `.screen` sample buffers. Convert permission denial and stopped-stream errors into typed errors and finish the frame stream.
...
```bash
git add Foldy/Capture Foldy/Overlay Foldy/Rendering/FoldRenderer.swift Foldy/App/AppModel.swift FoldyTests
...
**Files:**
- Create: `Foldy/Settings/AppSettings.swift`
- Create: `Foldy/Settings/LaunchAtLoginService.swift`
- Create: `Foldy/Entitlement/EntitlementStore.swift`
- Create: `Foldy/Entitlement/LocalEntitlementStore.swift`
- Create: `Foldy/Entitlement/DirectLicenseStore.swift`
- Create: `Foldy/UI/OnboardingView.swift`
- Modify: `Foldy/UI/MenuBarContentView.swift`
- Modify: `Foldy/App/AppModel.swift`
- Test: `FoldyTests/AppSettingsTests.swift`
- Test: `FoldyTests/EntitlementStoreTests.swift`
...
Verify defaults are effect enabled false, start angle 110°, intensity 1.0, launch at login false, onboarding incomplete. Verify a development entitlement unlocks only when the `FOLDY_DEVELOPMENT_UNLOCK` build setting is true. Verify an invalid direct license remains locked.
...
```bash
git add Foldy/Settings Foldy/Entitlement Foldy/UI Foldy/App/AppModel.swift FoldyTests
...
- Create: `README.md`
- Modify: `Foldy/Resources/Assets.xcassets/AppIcon.appiconset/Contents.json`
- Create: generated icon PNGs under `Foldy/Resources/Assets.xcassets/AppIcon.appiconset/`

**Interfaces:**
- Consumes: the complete `Foldy` target.
- Produces: an unsigned local `Foldy.app`, a Release archive/export path, and an optional notarized `Foldy.dmg` when valid owner credentials are supplied.
...
```bash
xcodegen generate
xcodebuild clean test -project Foldy.xcodeproj -scheme Foldy -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO
xcodebuild build -project Foldy.xcodeproj -scheme Foldy -configuration Release -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO
./scripts/package.sh
git status --short
```
...
```bash
git add scripts Config README.md Foldy/Resources
git commit -m "build: package and document Foldy release"
```
